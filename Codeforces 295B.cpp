...
int main(){
    int n;
    lli ans[510], x[510], dp[510][510], cnt;
    scanf("%d", &n);
    for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++) scanf("%lld", &dp[i][j]);
    for(int i = n; i >= 1; i--) scanf("%lld", &x[i]);
    for(int k = 1; k <= n; k++){
        cnt=0;
        for(int i = 1; i <= n; i++){
            for(int j = 1; j <= n; j++){
                if(dp[x[i]][x[k]]+dp[x[k]][x[j]] < dp[x[i]][x[j]]) dp[x[i]][x[j]]=dp[x[i]][x[k]]+dp[x[k]][x[j]];
                if(i <= k && j <= k) cnt+=dp[x[i]][x[j]];
            }
        }
        ans[k]=cnt;
    }
